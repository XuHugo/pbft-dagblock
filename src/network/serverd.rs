...
async fn handle_req(
    ...
    println!(
        "[{} Node{}] Received RequestMsg: {:?}",
        emoji, server_data.node.id, request_msg
    );
    if server_data.node.is_faulty {
    ...
    server_data
        ...
        .lock()
        .unwrap()
        .push(request_msg.clone());
    {
        let mut current_stage_guard = server_data.node.current_state.current_stage.lock().unwrap();
        if *current_stage_guard == crate::consensus::pbft::Stage::Idle {
            *current_stage_guard = crate::consensus::pbft::Stage::PrePrepare;
        }
    }
    println!(
        "🌟[{} Node{}] Transitioned to PrePrepare stage!",
    ...
    if server_data.node.id == server_data.node.view.primary_node_id {
        ...
        let requests: Vec<_> = server_data
            .node
            .node_table
            .iter()
            .filter_map(|(id, url)| {
            ...
            .collect();
        {
            let mut current_stage_guard =
                server_data.node.current_state.current_stage.lock().unwrap();
            if *current_stage_guard == crate::consensus::pbft::Stage::PrePrepare {
                *current_stage_guard = crate::consensus::pbft::Stage::Prepare;
            }
        }
        println!(
            "🌟🌟[{} Primary Node{}] Transitioned to Prepare stage!",
            emoji, server_data.node.id
        );
...
async fn handle_pre_prepare(
    pre_prepare_msg: web::Json<crate::consensus::message::PrePrepareMsg>,
    server_data: Data<ServerD>,
) -> impl Responder {
    let response_body = json!({"status": "ok"});
    let mut emoji = "😃";
    ...
    println!(
        "[{} Node{}] Received PrePrepareMsg: {:?}",
        emoji, server_data.node.id, pre_prepare_msg
    );
    if server_data.node.is_faulty {
        return HttpResponse::Ok().json(response_body);
    }
    let mut request_msg = RequestMsg {
        // request message corresponding to the pre-prepare message
        time_stamp: 0,
        client_id: 0,
    ...
    if request_msg.digest == "" {
        eprintln!(
            "[{} Node{}] Request message not found for PrePrepareMsg",
            emoji, server_data.node.id
        );
        return HttpResponse::Ok().json(response_body);
    }
    let verify_result = crate::network::utils::verify_msg(
        &pre_prepare_msg,
        server_data.node.view.id,
        request_msg.digest.clone(),
    );
    if !verify_result {
        eprintln!(
            "[{} Node{}] PrePrepareMsg verification failed",
            emoji, server_data.node.id
        );
        HttpResponse::Ok().json(response_body)
    } else {
        ...
        server_data
            ...
            .lock()
            .unwrap()
            .push(pre_prepare_msg.into_inner());
        ...
        println!(
            "🌟🌟[{} Node{}] Transitioned to Prepare stage!",
            emoji, server_data.node.id
        );
        ...
        server_data
            ...
            .lock()
            .unwrap()
            .push(prepare_msg.clone()); // save the prepare message
        let client = reqwest::Client::new();
        let requests: Vec<_> = server_data
            .node
            .node_table
            ...
            .filter_map(|(id, url)| {
                ...
                Some(async move {
                    ...
                    match client_clone
                        .post(&format!("http://{}/prepare", url_clone))
...
async fn handle_prepare(
    prepare_msg: web::Json<crate::consensus::message::VoteMsg>,
    server_data: Data<ServerD>,
) -> impl Responder {
    let response_body = json!({"status": "ok"});
    let mut emoji = "😃";
    ...
    println!(
        "[{} Node{}] Received PrepareMsg: {:?}",
        emoji, server_data.node.id, prepare_msg
    );
    if server_data.node.is_faulty {
        return HttpResponse::Ok().json(response_body);
    }
    ...
    if preprepare_msg.digest == "" {
        eprintln!(
            "[{} Node{}] PrePrepareMsg not found for PrepareMsg",
            emoji, server_data.node.id
        );
        return HttpResponse::Ok().json(response_body);
    } else {
            ...
            let requests: Vec<_> = server_data
                ...
                .filter_map(|(id, url)| {
                    ...
                    Some(async move {
                        ...
                        match client_clone
                            .post(&format!("http://{}/commit", url_clone))
...
async fn handle_commit(
    commit_msg: web::Json<crate::consensus::message::VoteMsg>,
    server_data: Data<ServerD>,
) -> impl Responder {
    let response_body = json!({"status": "ok"});
    let mut emoji = "😃";
    ...
    println!(
        "[{} Node{}] Received CommitMsg: {:?}",
        emoji, server_data.node.id, commit_msg
    );
    if server_data.node.is_faulty {
        return HttpResponse::Ok().json(response_body);
    }
    ...
    if cnt == 2 * f + 1 {
        ...
        if request_msg.digest != "" {
            ...
            match client
                .post("http://127.0.0.1:9000/reply")
                .json(&reply_msg)
                .send()
                .await
            {
                Ok(response) => {
                    println!(
                        "[{} Node{}] Response from client server: {:?}",
                        emoji,
                        server_data.node.id,
                        response.status()
                    );
                ...
                Err(e) => {
                    eprintln!(
                        "[{} Node{}] Error sending request to client server: {}",
                        emoji, server_data.node.id, e
                    );
                }
            }
        }
...
fn start_server(port: u16, server_data: Arc<ServerD>) -> io::Result<()> {
    actix_web::rt::System::new().block_on(async move {
        ...
        println!("Normal Server started on port {}", port);
