...
use blockdag::{
    calc_blue, dag_blue_print, dag_print, dag_red_print, get_stpq, handle_block_rx, node_add_block,
    remove_past_future, sorted_keys_by_height, tips_anticone, update_tips,
};
use blockdag::{BlockRaw, Node};
use sync::{start_consensus, start_new_block};

pub fn start_nodes_dag(f: i32) {
    let _ = env_logger::try_init();

    //  f  how many nodes to simulate. each node is a thread spawn.
    ...
    println!(
        "start_nodes_dag(): start. k={}, blocks={}, nodes={}",
        K, blocks_generating, f
    );

    // important note: the token-ring locker must be drop as soon as possible by node.
    let block_token_ring: Arc<RwLock<HashMap<String, Arc<RwLock<BlockRaw>>>>> =
        ...
        let handle = thread::spawn(move || {
            ...
            loop {
                ...
                *blocks_generated_w += 1;
                ...
                //Select K+1 nodes with high scores.
                ...
                // propagate this new mined block
                {
                    ...
                    let prev_names = new_mined_block
                        .prev
                        .iter()
                        .map(|(k, _)| k.clone())
                        .collect::<Vec<String>>();
                ...
                latest_block_hash_clone
                    .store(block_name.parse::<isize>().unwrap(), Ordering::Relaxed);
                info!(
                    "{} new mined block: {}. height={},size_of_dag={}. mined_blocks={}",
                    node_w.name, block_name, node_w.height, node_w.size_of_dag, node_w.mined_blocks
                );
    ...
    let _dispatcher_handle = thread::spawn(move || {
        ...
        'dis_outer: loop {
            ...
            info!(
                "dispatcher recv block: {}. remaining queue size={}",
                new_block.name,
                to_be_dispatched.len()
            );

            let mut is_dispatched: HashMap<i32, bool> = HashMap::new();
            for i in 0..f {
                is_dispatched.insert(i, false);
            }
            ...
            for (name, block) in &to_be_dispatched {
                ...
                if is_dispatched_clone.len() == 0 {
                    finished_block_list.push(name.clone());
                }

                dispatched.insert(name.clone(), is_dispatched_clone);

                if new_block_arriving == true {
                    break;
                }
            }
    ...
    loop {
        ...
        (*mining).1 = height;
        println!(
            "⛏️start mining {} blocks at height {}. mining_lock={:?}",
            blocks_one_time, height, *mining
        );
        drop(mining);
...

        {
            ...
            if (*blocks_generated_r % 100) == 0 {
                println!("🛠️ send to consensus node{:?}!!!", *blocks_generated_r);
                let _ = start_consensus(*blocks_generated_r as u32);
            }
        }
    }
}
...
pub fn test_pfbf_dag(_n: u32) {
    ...
    println!(
        ...
        K, blocks_generating, TOTAL_NODES
    );
    ...
    loop {
        ...
        (*mining).1 = height;
        debug!(
            "test_nodes_sync(): start mining {} blocks at height {}. mining_lock={:?}",
            blocks_one_time, height, *mining
        );
        drop(mining);
    ...
    // println!(
    //     "test_nodes_performance(): done. block propagation hashmap remaining size: {}",
    //     propagation.len()
    // );
...
mod tests {
    ...
    use crate::blockdag::{
        calc_blue, dag_blue_print, dag_print, dag_red_print, get_stpq, handle_block_rx,
        node_add_block, remove_past_future, sorted_keys_by_height, tips_anticone, update_tips,
    };
    use crate::blockdag::{BlockRaw, Node};
    ...
    fn test_fig0() {
        ...
        assert_eq!(
            &blue_selection,
            "blues={Genesis,B,C,D,F,H,J,K,M,N,} total=10/13"
        );
    ...
    fn test_fig1() {
        ...
        let node = Node::init("fig1");

        let mut node_w = node.write().unwrap();

        macro_rules! dag_add {
            ( block=$a:expr, references=$b:expr ) => {
                node_add_block($a, $b, &mut node_w, k, true)
            };
        }
        ...
        assert_eq!(
            &blue_selection,
            "blues={Genesis,B,C,D,F,I,J,K,M,O,P,R,} total=12/20"
        );
    ...
    fn test_anticone() {
        ...
        let result = format!(
            "anticone of {} = {:?}",
            "H",
            sorted_keys_by_height(&anticone, false)
        );
        println!("{}", result);
        assert_eq!(
            result,
            "anticone of H = [(\"B\", 1), (\"F\", 2), (\"I\", 2)]"
        );
    ...
    #[test]
    fn test_add_block() {
        let blocks_generating: i32 = 10000;
        ...
        println!(
            "node=\"{}\",height={},size_of_dag={}",
            node_w.name, node_w.height, node_w.size_of_dag
        );
    ...
    fn test_fig_x1() {
        ...
        if k == 0 {
            assert_eq!(
                &blue_selection,
                "blues={Genesis,B,01,05,09,11,13,14,16,18,23,26,30,36,45,} total=15/50"
            )
        } else {
            assert_eq!(2 + 2, 4);
        }
    ...
    fn test_fig_x3() {
        ...
        dag_add!(
            block = "0067",
            references = &vec!["0061", "0066", "0063", "0060"]
        );
        ...
        dag_add!(
            block = "0082",
            references = &vec!["0067", "0069", "0072", "0075"]
        );
    ...
    fn test_nodes_performance() {
            ...
            let handle = thread::spawn(move || {
                ...
                loop {
                    ...
                    *blocks_generated_w += 1;
                    ...
                    score_stpq.truncate((K + 1) as usize);
                    let references_str =
                        score_stpq.iter().map(|&(ref s, _, _)| s.as_ref()).collect();
                    node_add_block(&block_name, &references_str, &mut node_w, K, true);
        ...
        println!("========================test done.==================================");
        println!(
            "test_nodes_performance(): start. k={}, blocks={}, nodes={}",
            K, blocks_generating, TOTAL_NODES
        );
