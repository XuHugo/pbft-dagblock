...
///
pub fn tips_anticone(
    tip_name: &str,
    tips: &HashMap<String, Arc<RwLock<Block>>>,
) -> HashMap<String, Arc<RwLock<Block>>> {
    ...
    let mut rest_maxmin = MaxMin {
        max: 0,
        min: <u64>::max_value(),
    };
    let mut maxi_maxmin = MaxMin {
        max: 0,
        min: <u64>::max_value(),
    };
    ...
    while rest_pred_set.len() > 0 && iteration_steps < ANTICONE_MAX_ITERATION {
        ...
        let _rest_local_maxmin = step_one_past(
            &rest_pred_set,
            &mut new_rest_pred,
            &mut used_rest,
            &mut rest_maxmin,
        ...
        loop {
            ...
            let max_local_maxmin = step_one_past(
                &maxi_pred_set,
                &mut new_maxi_pred,
                &mut used_maxi,
                &mut maxi_maxmin,
    ...
    if iteration_steps >= ANTICONE_MAX_ITERATION {
        warn!(
            "tips_anticone(): tip={}. too many iterations! force to break.",
            tip_name
        );
    }
...
///
pub fn anticone_blue(
    any_name: &str,
    node: &Node,
    tips: &HashMap<String, Arc<RwLock<Block>>>,
    k: i32,
) -> (i32, HashMap<String, Arc<RwLock<Block>>>) {
    ...
    debug!(
        "anticone_blue(): k={}. virtual tips={:?}",
        k,
        sorted_keys_by_height(&virtual_tips, false)
            .iter()
            .map(|&(ref n, _)| { n })
            .collect::<Vec<_>>()
    );
    ...
    // left half
    let (anticone_blue_count_left, mut anticone_left) =
        tips_anticone_blue(any_name, &virtual_tips, k);
    debug!(
        "anticone_blue(): left half anticone_blue_count={}, anticone_blue={:?}",
        anticone_blue_count_left,
        sorted_keys_by_height(&anticone_left, true)
            .iter()
            .map(|&(ref n, _)| { n })
            .collect::<Vec<_>>()
    );
    ...
    // right half
    let (anticone_blue_count_right, anticone_right) =
        tips_anticone_blue_rev(any_name, &virtual_tips, k - anticone_blue_count_left);
    debug!(
        "anticone_blue(): right half anticone_blue_count={}",
        anticone_blue_count_right
    );
    append_maps(&mut anticone_left, &anticone_right);

    return (
        anticone_blue_count_left + anticone_blue_count_right,
        anticone_left,
    );
}
...
///
pub fn tips_anticone_blue(
    tip_name: &str,
    tips: &HashMap<String, Arc<RwLock<Block>>>,
    k: i32,
) -> (i32, HashMap<String, Arc<RwLock<Block>>>) {
    debug!(
        "tips_anticone_blue(): tip={} func enter. tips={:?}",
        tip_name,
        sorted_keys_by_height(tips, true)
            .iter()
            .map(|&(ref n, _)| { n })
            .collect::<Vec<_>>()
    );
    ...
    if tips.len() == 0 {
        error!(
            "tips_anticone_blue(): tip={} error! tips is empty",
            tip_name
        );
        return (-1, anticone);
    }
    ...
    if maxi_pred_set.len() == 0 {
        error!(
            "tips_anticone_blue(): error! tip {} is not in tips",
            tip_name
        );
    ...
    debug!(
        "tips_anticone_blue(): tip={} size_of_anticone_blue={}",
        tip_name,
        anticone.len()
    );
    ...
    while rest_pred_set.len() > 0 && anticone_blue_count <= k {
        ...
        rest_pred_set = new_rest_pred;
        debug!(
            "tips_anticone_blue(): tip={} size_of_anticone={} rest_pred_set={}",
            tip_name,
            anticone.len(),
            rest_pred_set.len()
        );
    ...
    debug!(
        "tips_anticone_blue(): tip={} final result: size_of_anticone={}",
        tip_name,
        anticone.len()
    );
...
pub fn tips_anticone_blue_rev(
    ...
    if tips.len() == 0 {
        error!(
            "tips_anticone_blue_rev(): tip={} error! tips is empty",
            tip_name
        );
        return (-1, anticone);
    }
    ...
    if maxi_pred_set.len() == 0 {
        error!(
            "tips_anticone_blue_rev(): error! tip {} is not in tips",
            tip_name
        );
        return (-1, HashMap::new());
    }
    ...
    debug!(
        "tips_anticone_blue_rev(): tip={} size_of_anticone_blue={}",
        tip_name,
        anticone.len()
    );
    ...
    while rest_pred_set.len() > 0 && anticone_blue_count <= k {
        let mut new_rest_pred: HashMap<String, Arc<RwLock<Block>>> = HashMap::new();
        let _rest_local_maxmin = step_one_next(
            &rest_pred_set,
            &mut new_rest_pred,
            &mut used_rest,
            &mut rest_maxmin,
        ...
        loop {
            let mut new_maxi_pred: HashMap<String, Arc<RwLock<Block>>> = HashMap::new();
            let max_local_maxmin = step_one_next(
                &maxi_pred_set,
                &mut new_maxi_pred,
                &mut used_maxi,
                &mut maxi_maxmin,
        ...
        rest_pred_set = new_rest_pred;
        debug!(
            "tips_anticone_blue_rev(): tip={} size_of_anticone_blue={} rest_pred_set={}",
            tip_name,
            anticone.len(),
            rest_pred_set.len()
        );
    ...
    debug!(
        "tips_anticone_blue_rev(): tip={} final result: size_of_anticone_blue={}",
        tip_name,
        anticone.len()
    );
...
fn step_one_next(
    pred: &HashMap<String, Arc<RwLock<Block>>>,
    new_pred: &mut HashMap<String, Arc<RwLock<Block>>>,
    used: &mut HashMap<String, bool>,
    maxmin: &mut MaxMin,
) -> MaxMin {
    let mut local_maxmin = MaxMin {
        max: 0,
        min: <u64>::max_value(),
    };
