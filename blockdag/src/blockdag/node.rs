...
use crate::blockdag::{calc_blue, dag_add_block, sorted_keys_by_height};
use crate::blockdag::{Block, BlockRaw};
...
impl fmt::Display for Node {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let mut formatted_info = format!(
            "node={},height={},size_of_dag={},dag={{",
            self.name, self.height, self.size_of_dag
        );
...
pub fn handle_block_rx(
    block_rx: BlockRaw,
    node: &mut Node,
    stash: &mut HashMap<String, BlockRaw>,
    k: i32,
) {
...
pub fn node_add_block(
    name_of_new_block: &str,
    references: &Vec<&str>,
    node: &mut Node,
    k: i32,
    do_update_tips: bool,
    ...
    // add block
    {
        ...
        if block.is_some() {
            ...
            // classmates update
            let classmate = classmates
                .entry(block.height)
                .or_insert(vec![name_of_new_block.clone().into()]);
