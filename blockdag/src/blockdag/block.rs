...
impl fmt::Display for Block {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let mut formated_info = if self.is_blue {
            format!(
                "name={},height={},size_of_past_set={},size_of_past_blue={},blue=1,prev=",
                self.name, self.height, self.size_of_past_set, self.size_of_past_blue
            )
        } else {
            format!(
                "name={},height={},size_of_past_set={},size_of_past_blue={},blue=0,prev=",
                self.name, self.height, self.size_of_past_set, self.size_of_past_blue
            )
        };

        formated_info.push_str(&format!(
            "{:?}",
            sorted_keys_by_height(&self.prev, false)
                .iter()
                .map(|&(ref n, _)| { n })
                .collect::<Vec<_>>()
        ));
...
pub fn append_maps(
    target: &mut HashMap<String, Arc<RwLock<Block>>>,
    source: &HashMap<String, Arc<RwLock<Block>>>,
) {
...
pub fn sorted_keys_by_height(
    ...
    if reverse == true {
        keys_vec.sort_by(|a, b| match a.1.cmp(&b.1).reverse() {
            Ordering::Equal => a.0.cmp(&b.0),
            other => other,
        });
    } else {
        keys_vec.sort_by(|a, b| match a.1.cmp(&b.1) {
            Ordering::Equal => a.0.cmp(&b.0),
            other => other,
        });
    }
...
pub fn get_stpq(source: &HashMap<String, Arc<RwLock<Block>>>) -> Vec<(String, u64, u64)> {
    ...
    for (_key, value) in source {
        ...
        keys_vec.push((
            String::from(block.name.clone()),
            block.size_of_past_blue,
            block.size_of_past_set,
        ));
    }

    keys_vec.sort_by(|a, b| match a.1.cmp(&b.1).reverse() {
        Ordering::Equal => match a.2.cmp(&b.2).reverse() {
            Ordering::Equal => a.0.cmp(&b.0),
            other => other,
        },
        other => other,
