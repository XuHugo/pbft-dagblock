...
use crate::blockdag::{
    anticone_blue, get_ltpq, get_stpq, sizeof_pastset, sorted_keys_by_height, tips_anticone,
    tips_anticone_blue, Block, Node,
};
...
pub fn calc_blue(block_name: &str, node: &mut Node, k: i32) {
    ...
    debug!(
        "calc_blue(): block {}.tip_max_name={},max_past_blue={}. tips={:?}",
        block_name,
        score_stpq[0].0,
        score_stpq[0].1,
        sorted_keys_by_height(tips, false)
            .iter()
            .map(|&(ref n, _)| { n })
            .collect::<Vec<_>>()
    );
    ...
    if &score_stpq[0].0 == block_name {
        debug!(
            "calc_blue(): step 3. block {}. new block is the max past blue",
            block_name
        );
        ...
        for &(ref name, _, _) in &score_stpq {
            ...
            if blues < 0 || blues > k {
                debug!(
                    "calc_blue(): block {}. tip {} size_of_anticone_blue={} not blue.",
                    block_name, name, blues
                );
            } else {
                // step 7
                {
        ...
        for &(ref name, _) in &prev_keys {
            ...
            debug!(
                "calc_blue(): step 6. block {}. come to block {}",
                block_name, name
            );
            {
                // step 10
        ...
        debug!(
            "calc_blue(): block {}. new block is not the max past blue",
            block_name
        );
        ...
        debug!(
            "calc_blue(): step 11. block {}. size_of_anticone_blue={}",
            block_name, blues
        );
        if blues >= 0 && blues <= k {
            let mut block_w = dag.get(block_name).unwrap().write().unwrap();
